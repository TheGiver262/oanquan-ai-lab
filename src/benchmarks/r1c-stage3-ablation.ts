...
const positions = BALANCED_MIDGAME_POSITIONS.filter((position) => STAGE3_IDS.has(position.id));
if (positions.length !== STAGE3_IDS.size) throw new Error(`Expected ${STAGE3_IDS.size} Stage 3 positions, got ${positions.length}`);
for (const position of positions) {
  const state = replayBalancedPosition(position);
  const totalDan = state.pits.filter((pit) => pit.kind === "dan").reduce((sum, pit) => sum + pit.stones, 0);
  const quanAlive = state.pits.filter((pit) => pit.kind === "quan" && pit.quanStones > 0).length;
  if (totalDan > 10 || quanAlive !== 0) throw new Error(`Stage 3 structural invariant failed for ${position.id}`);
}
...
const result = {
  experiment: "R1c-stage3-low-material-cycle-ablation",
  evidenceClass: candidate === baseline ? "null-control" : "same-family-fixed-simulation",
  ruleset: "oaq:classic_2p:standard:v1",
  methodology: {
    candidate,
    baseline,
    corpus: "preselected from structural audit only: totalDan <= 10, no Quan alive, cycle-closing edge observed in bounded DFS",
    ...
    cyclePolicy: "Repetition is not an automatic draw.",
  },
  config: { simulations, additionalMaxMoves, puctExploration: 1.5, policyTemperature: 0.6 },
  positions: positions.map((position) => structuralRow(position)),
...
function structuralRow(position: BalancedMidgamePosition) {
  const state = replayBalancedPosition(position);
  const totalDan = state.pits.filter((pit) => pit.kind === "dan").reduce((sum, pit) => sum + pit.stones, 0);
  return { id: position.id, depth: position.depth, source: position.source, totalDan, scoreDiff: position.scoreDiff, legalMoves: position.legalMoves };
}

function play(position: BalancedMidgamePosition, candidateSeat: PlayerId): GameResult {
  let state = replayBalancedPosition(position);
  ...
  while (state.status === "playing" && state.moveNumber < limit) {
    ...
    if (engine === "puct-v3a") {
      ...
      move = session.chooseMove(state, { simulations, puctExploration: 1.5, policyTemperature: 0.6 }).move;
    } else {
...
function unresolved(position: BalancedMidgamePosition, candidateSeat: PlayerId, continuationMoves: number): GameResult {
