...
const result = {
  ...
  methodology: {
    ...
    corpus: "score-balanced midgame states derived from frozen 50M-node V3 principal variations",
    pairedPositions: "Each exact midgame state is played twice with research/opponent engine ownership swapped between P0 and P1.",
    moveCap: `${additionalMaxMoves} additional plies after each forced midgame prefix`,
  ...
  opponent: "puct-v3a",
  positionCount: BALANCED_MIDGAME_POSITIONS.length,
  positions: BALANCED_MIDGAME_POSITIONS.map((position) => ({
    id: position.id,
...
function playGame(
  position: BalancedMidgamePosition,
  pair: number,
  researchSeat: PlayerId,
): GameResult {
