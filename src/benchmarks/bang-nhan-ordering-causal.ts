...
import {
  ...
  type BangNhanOrderingMode,
} from "../research/bang-nhan-ordering-experiment.js";
...
const aggregate = {
  ...
  gameDetails: [] as Array<{
    ...
    unresolved: boolean;
    moves: number;
    bangNhanAvgDepth: number;
    bangNhanAvgNodes: number;
  }>,
};

for (let index = 0; index < games; index += 1) {
  const mctsSeat: PlayerId = index % 2 === 0 ? "P0" : "P1";
  const seed = seedBase + index * 7919 + hashString(`${variant}:bang-nhan:production-max`);
...
function playGame(gameNumber: number, mctsSeat: PlayerId, seed: number) {
  ...
  while (state.status === "playing" && state.moveNumber < 160) {
    ...
    if (player === mctsSeat) {
      ...
      const decision = chooseBangNhanExperimentMove(state, player, {
        orderingMode: "fixed-opponent-ordering" satisfies BangNhanOrderingMode,
        timeBudgetMs: 900,
        random: productionRandom,
      });
      depths.push(decision.completedDepth);
...
function summarize(
  ...
  return {
    ...
    unresolved,
    moves,
    bangNhanAvgDepth: average(depths),
    bangNhanAvgNodes: average(nodes),
  };
}
