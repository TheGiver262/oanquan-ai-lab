...
import { chooseIterativeOrderingExperimentMove } from "../src/research/iterative-ordering-experiment.js";
import type { GameState } from "../src/types.js";

describe("Thám Hoa ordering experiment parity", () => {
  it("matches server production decisions before enabling the ordering fix", () => {
    const profile = PRODUCTION_TOP_PROFILES["tham-hoa"];
    ...
    try {
      for (const [index, state] of corpus().entries()) {
        const server = chooseServerProductionMoveWithDiagnostics(state, "tham-hoa", state.currentPlayer, {
          mode: "production-max",
          now: () => 0,
          random: mulberry32(2000 + index),
          timeBudgetMs: 60_000,
        });
        const experiment = chooseIterativeOrderingExperimentMove(state, "tham-hoa", state.currentPlayer, {
...
function corpus(): GameState[] {
  ...
  for (let ply = 0; ply < 22 && state.status === "playing"; ply += 1) {
    const moves = getLegalMoves(state);
    const move = moves[(ply * 5 + 2) % moves.length];
    if (!move) break;
